...
#include <snax/ignore.hpp>
#include <snax/symbol.hpp>
...
using snax::ignore;
using snax::ignore_wrapper;
using snax::pack;
using snax::pack_size;
using snax::public_key;
using snax::signature;
using snax::symbol;
using snax::symbol_code;
using snax::unpack;
...
struct be_test {
   ...
   be_test(int i) : val{i} {}
   int val;
   SNAXLIB_SERIALIZE( be_test, (val) )
};

// Definitions in `snax.cdt/libraries/snax/datastream.hpp`
SNAX_TEST_BEGIN(datastream_test)
   silence_output(true);
...
// Definitions in `snax.cdt/libraries/snax/datastream.hpp`
SNAX_TEST_BEGIN(datastream_specialization_test)
   silence_output(true);
...
// Definitions in `snax.cdt/libraries/snax/datastream.hpp`
SNAX_TEST_BEGIN(datastream_stream_test)
   silence_output(true);
...
// Definitions in `snax.cdt/libraries/snax/datastream.hpp`
SNAX_TEST_BEGIN(misc_datastream_test)
   silence_output(true);
...
int main(int argc, char* argv[]) {
   SNAX_TEST(datastream_test);
   SNAX_TEST(datastream_specialization_test);
   SNAX_TEST(datastream_stream_test);
   SNAX_TEST(misc_datastream_test);
   return has_failed();
}
