...
// Definitions in `snax.cdt/libraries/snax/symbol.hpp`
SNAX_TEST_BEGIN(symbol_code_type_test)
   silence_output(true);
...
// Definitions in `snax.cdt/libraries/snax/symbol.hpp`
SNAX_TEST_BEGIN(symbol_type_test)
   silence_output(true);
...
// Definitions in `snax.cdt/libraries/snax/symbol.hpp`
SNAX_TEST_BEGIN(extended_symbol_type_test)
   silence_output(true);
...
int main(int argc, char* argv[]) {
   SNAX_TEST(symbol_code_type_test);
   SNAX_TEST(symbol_type_test);
   SNAX_TEST(extended_symbol_type_test);
   return has_failed();
}
