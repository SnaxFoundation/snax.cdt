...
#include <snax/transaction.hpp>

#include "transfer.hpp" 

using namespace snax;

class [[snax::contract]] simple_tests : public contract {
   public:
      ...
      [[snax::action("test4")]] 
      void test4(name to) {
         transfer_contract::transfer_action trans("snax.token"_n, {_self, "active"_n});
      ...
      void on_transfer(name from, name to, asset quant, std::string memo) {
         check(get_first_receiver() == "snax.token"_n, "should be snax.token");
      ...
      void on_transfer2(name from, name to, asset quant, std::string memo) {
         check(get_first_receiver() != "snax.token"_n, "should not be snax.token");
         print_f("On notify 2 : % % % %", from, to, quant, memo);
      }

      [[snax::on_notify("*::transfer2")]] 
...
extern "C" void post_dispatch(name self, name original_receiver, name action) {
   ...
   check(self == "snax"_n, "should only be snax for action failures");
}
