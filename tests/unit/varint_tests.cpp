...
#include <snax/varint.hpp>

using std::numeric_limits;

using snax::datastream;
using snax::unsigned_int;
using snax::signed_int;
...
// Defined in `snax.cdt/libraries/snax/varint.hpp`
SNAX_TEST_BEGIN(unsigned_int_type_test)
...
// Defined in `snax.cdt/libraries/snax/varint.hpp`
SNAX_TEST_BEGIN(signed_int_type_test)
   silence_output(false);
...
int main(int argc, char* argv[]) {
   SNAX_TEST(unsigned_int_type_test)
   SNAX_TEST(signed_int_type_test);
   return has_failed();
}
