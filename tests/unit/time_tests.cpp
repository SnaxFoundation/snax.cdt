...
#include <snax/time.hpp>

using std::numeric_limits;

using snax::days;
using snax::hours;
using snax::microseconds;
using snax::milliseconds;
using snax::minutes;
using snax::seconds;
using snax::time_point;
using snax::time_point_sec;
...
// Definitions in `snax.cdt/libraries/snax/time.hpp`
SNAX_TEST_BEGIN(microseconds_type_test)
silence_output(true);
...
// Definitions in `snax.cdt/libraries/snax/time.hpp`
SNAX_TEST_BEGIN(time_point_type_test)
   silence_output(true);
...
// Definitions in `snax.cdt/libraries/snax/time.hpp`
SNAX_TEST_BEGIN(time_point_sec_type_test)
   silence_output(true);
...
// Definitions in `snax.cdt/libraries/snax/time.hpp`
SNAX_TEST_BEGIN(block_timestamp_type_test)
   silence_output(true);
...
int main(int argc, char* argv[]) {
   SNAX_TEST(microseconds_type_test);
   SNAX_TEST(time_point_type_test);
   SNAX_TEST(time_point_sec_type_test);
   SNAX_TEST(block_timestamp_type_test);
   return has_failed();
}
