...
namespace snax {
   ...
   */
   ...
   public:
      ...
      {
         if( str.size() > 7 ) {
            snax::check( false, "string is too long to be a valid symbol_code" );
         }
         for( auto itr = str.rbegin(); itr != str.rend(); ++itr ) {
            if( *itr < 'A' || *itr > 'Z') {
               snax::check( false, "only uppercase letters allowed in symbol_code string" );
            }
            value <<= 8;
            value |= *itr;
         }
   ...
   inline DataStream& operator<<(DataStream& ds, const snax::symbol_code sym_code) {
    ...
    */
   template<typename DataStream>
   inline DataStream& operator>>(DataStream& ds, snax::symbol_code& sym_code) {
     uint64_t raw = 0;
   ...
   public:
      ...
      void print( bool show_precision = true )const {
         if( show_precision ){
            ::snax::print( static_cast<uint64_t>(precision()), "," );
         }
         char buffer[7];
         auto end = code().write_as_string( buffer, buffer + sizeof(buffer) );
         if( buffer < end )
            ::snax::print( buffer, (end-buffer) );
      }
    ...
    */
   template<typename DataStream>
   inline DataStream& operator<<(DataStream& ds, const snax::symbol sym) {
    ...
    */
   template<typename DataStream>
   inline DataStream& operator>>(DataStream& ds, snax::symbol& sym) {
     uint64_t raw = 0;
     ds.read((char*)&raw, sizeof(raw));
     sym = symbol(raw);
     return ds;
   ...
   private:
      ...
      SNAXLIB_SERIALIZE( extended_symbol, (symbol)(contract) )
   };
}
