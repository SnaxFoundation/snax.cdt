...
#warning "<snaxlib/system.hpp> is deprecated use <snax/system.hpp>"
namespace snax {
   ...
   inline void check(bool pred, const char* msg, size_t n) {
      if (!pred) {
         snax_assert_message(false, msg, n);
      }
   }
   ...
   inline void check(bool pred, const std::string& msg, size_t n) {
      if (!pred) {
         snax_assert_message(false, msg.c_str(), n);
      }
   }
