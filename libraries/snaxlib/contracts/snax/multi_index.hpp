...
#include "../../contracts/snax/action.hpp"
#include "../../core/snax/name.hpp"
#include "../../core/snax/serialize.hpp"
#include "../../core/snax/fixed_bytes.hpp"
...
namespace _multi_index_detail {
   ...
   WRAP_SECONDARY_ARRAY_TYPE(idx256, snax::fixed_bytes<32>)
   template<>
   struct secondary_key_traits<snax::fixed_bytes<32>> {
      static constexpr snax::fixed_bytes<32> true_lowest() { return snax::fixed_bytes<32>(); }
   };
 ...
 *      }
 *  }
 *  SNAX_DISPATCH( mycontract, (myaction) )
 *  @endcode
 */
 ...
 *  @details SNAX Multi-Index API provides a C++ interface to the SNAX database. It is patterned after Boost Multi Index Container.
...
class multi_index
{
   private:
      ...
      struct index {
         public:
            ...
            constexpr static bool validate_index_name( snax::name n ) {
               return n.value != 0 && n != snax::name("primary"); // Primary is a reserve index name.
            }
            ...
            struct const_iterator : public std::iterator<std::bidirectional_iterator_tag, const T> {
               public:
                  ...
                  const_iterator& operator++() {
                     ...
                     snax::check( _item != nullptr, "cannot increment end iterator" );
                  ...
                  const_iterator& operator--() {
                     ...
                     if( !_item ) {
                        ...
                        snax::check( ei != -1, "cannot decrement end iterator when the index is empty" );
                        prev_itr = secondary_index_db_functions<secondary_key_type>::db_idx_previous( ei , &prev_pk );
                        snax::check( prev_itr >= 0, "cannot decrement end iterator when the index is empty" );
                     } else {
                        if( _item->__iters[Number] == -1 ) {
                           secondary_key_type temp_secondary_key;
                           auto idxitr = secondary_index_db_functions<secondary_key_type>::db_idx_find_primary(_idx->get_code().value, _idx->get_scope(), _idx->name(), _item->primary_key(), temp_secondary_key);
                        ...
                        snax::check( prev_itr >= 0, "cannot decrement iterator at beginning of index" );
                     }
            ...
            const_iterator require_find( const secondary_key_type& secondary, const char* error_msg = "unable to find secondary key" )const {
               auto lb = lower_bound( secondary );
               snax::check( lb != cend(), error_msg );
               snax::check( secondary == secondary_extractor_type()(*lb), error_msg );
               return lb;
            }
            ...
            const T& get( const secondary_key_type& secondary, const char* error_msg = "unable to find secondary key" )const {
               ...
               snax::check( result != cend(), error_msg );
               return *result;
            }
            ...
            const_iterator iterator_to( const T& obj ) {
               ...
               snax::check( objitem.__idx == _multidx, "object passed to iterator_to is not in multi_index" );
            ...
            void modify( const_iterator itr, snax::name payer, Lambda&& updater ) {
               snax::check( itr != cend(), "cannot pass end iterator to modify" );

               _multidx->modify( *itr, payer, std::forward<Lambda&&>(updater) );
            }

            const_iterator erase( const_iterator itr ) {
               snax::check( itr != cend(), "cannot pass end iterator to erase" );
            ...
            snax::name get_code()const  { return _multidx->get_code(); }
      ...
      static constexpr auto transform_indices( ) {
         ...
         return hana::transform( indices_input_type(), [&]( auto&& idx ){
             ...
             return hana::make_tuple( hana::type_c<index<snax::name::raw(static_cast<uint64_t>(idx_type::index_name)),
                                                         typename idx_type::secondary_extractor_type,
                                                         num_type::e::value, false> >,
                                      hana::type_c<index<snax::name::raw(static_cast<uint64_t>(idx_type::index_name)),
                                                         typename idx_type::secondary_extractor_type,
      ...
      const item& load_object_by_primary_iterator( int32_t itr )const {
         ...
         snax::check( size >= 0, "error reading iterator" );
   ...
   public:
       ...
       *
       *  Notes
       *  The `snax::multi_index` template has template parameters `<name::raw TableName, typename T, typename... Indices>`, where:
       *  - `TableName` is the name of the table, maximum 12 characters long, characters in the name from the set of lowercase letters, digits 1 to 5, and the "." (period) character and is converted to a snax::raw - which wraps uint64_t;
       ...
       *      typedef snax::multi_index< "address"_n, address > address_index;
       *      void myaction() {
       *        address_index addresses(_self, _self.value); // code, scope
       *      }
       *  }
       *  SNAX_DISPATCH( addressbook, (myaction) )
       *  @endcode
       */
       ...
       *        snax::check(addresses.get_code() == "dan"_n, "Codes don't match.");
       *      }
       *  }
       *  SNAX_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *      void myaction() {
       *        address_index addresses("dan"_n, "dan"_n); // code, scope
       *        snax::check(addresses.get_code() == "dan"_n, "Scopes don't match");
       *      }
       *  }
       *  SNAX_DISPATCH( addressbook, (myaction) )
       *  @endcode
      ...
      struct const_iterator : public std::iterator<std::bidirectional_iterator_tag, const T> {
         ...
         const_iterator& operator--() {
            ...
            if( !_item ) {
               ...
               snax::check( ei != -1, "cannot decrement end iterator when the table is empty" );
               prev_itr = internal_use_do_not_use::db_previous_i64( ei , &prev_pk );
               snax::check( prev_itr >= 0, "cannot decrement end iterator when the table is empty" );
            } else {
               prev_itr = internal_use_do_not_use::db_previous_i64( _item->__primary_itr, &prev_pk );
               snax::check( prev_itr >= 0, "cannot decrement iterator at beginning of table" );
            }
       ...
       *        snax::check(itr == addresses.cbegin(), "Only address is not at front.");
       *      }
       *  }
       *  SNAX_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *
       *        auto itr = addresses.find("dan"_n);
       *        snax::check(itr == addresses.begin(), "Only address is not at front.");
       *      }
       *  }
       ...
       *
       *        auto itr = addresses.find("dan"_n);
       *        snax::check(itr != addresses.cend(), "Address for account doesn't exist");
       *      }
       *  }
       *  SNAX_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *
       *        auto itr = addresses.find("dan"_n);
       *        snax::check(itr != addresses.end(), "Address for account doesn't exist");
       *      }
       *  }
       *  SNAX_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *        snax::check(itr->account_name == name("dan"), "Lock arf, Incorrect Last Record ");
       *        itr++;
       *        snax::check(itr->account_name == name("brendan"), "Lock arf, Incorrect Second Last Record");
       *      }
       *  }
       *  SNAX_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *        itr--;
       *        snax::check(itr->account_name == name("brendan"), "Lock arf, Incorrect First Record ");
       *        itr--;
       *        snax::check(itr->account_name == name("dan"), "Lock arf, Incorrect Second Record");
       *      }
       *  }
       *  SNAX_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *        snax::check(itr->account_name == name("brendan"), "Lock arf, Incorrect First Lower Bound Record ");
       *        itr++;
       *        snax::check(itr->account_name == name("dan"), "Lock arf, Incorrect Second Lower Bound Record");
       *        itr++;
       *        snax::check(itr == zip_index.end(), "Lock arf, Incorrect End of Iterator");
       *      }
       *  }
       *  SNAX_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *        snax::check(itr->account_name == name("dan"), "Lock arf, Incorrect First Upper Bound Record ");
       *        itr++;
       *        snax::check(itr == zip_index.end(), "Lock arf, Incorrect End of Iterator");
       *      }
       *  }
      ...
      uint64_t available_primary_key()const {
         ...
         snax::check( _next_primary_key < no_available_primary_key, "next primary key in table is at autoincrement limit");
       ...
       *      typedef snax::multi_index< name("address"), address, indexed_by< name("zip"), const_mem_fun<address, uint64_t, &address::by_zip> > address_index;
       *      void myaction() {
       *        // create reference to address_index  - see emplace example below
       *        // add dan account to table           - see emplace example below
       ...
       *        snax::check(itr->account_name == name("dan"), "Lock arf, Incorrect Record ");
       *      }
       *  }
       *  SNAX_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *        snax::check(iterator_to(user) == itr, "Invalid iterator");
       *      }
       *  }
       *  SNAX_DISPATCH( addressbook, (myaction) )
       *  @endcode
      ...
      const_iterator iterator_to( const T& obj )const {
         const auto& objitem = static_cast<const item&>(obj);
         snax::check( objitem.__idx == this, "object passed to iterator_to is not in multi_index" );
         return {this, &objitem};
      }
      /**
      ...
      const_iterator emplace( name payer, Lambda&& constructor ) {
         ...
         snax::check( _code == current_receiver(), "cannot create objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
       ...
       *
       *        auto itr = addresses.find("dan"_n);
       *        snax::check(itr != addresses.end(), "Address for account not found");
      ...
      void modify( const_iterator itr, name payer, Lambda&& updater ) {
         snax::check( itr != end(), "cannot pass end iterator to modify" );
       ...
       *          address.state = "CA";
       *        });
       *        snax::check(itr->city == "San Luis Obispo", "Lock arf, Address not modified");
       *      }
       *  }
       *  SNAX_DISPATCH( addressbook, (myaction) )
       *  @endcode
      ...
      void modify( const T& obj, name payer, Lambda&& updater ) {
         ...
         const auto& objitem = static_cast<const item&>(obj);
         snax::check( objitem.__idx == this, "object passed to modify is not in multi_index" );
         auto& mutableitem = const_cast<item&>(objitem);
         snax::check( _code == current_receiver(), "cannot modify objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
         ...
         snax::check( pk == obj.primary_key(), "updater cannot change primary key when modifying an object" );
       ...
       *
       *        auto user = addresses.get("dan"_n);
       *        snax::check(user.first_name == "Daniel", "Couldn't get him.");
       *      }
       *  }
       *  SNAX_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *
       *        auto itr = addresses.find("dan"_n);
       *        snax::check(itr != addresses.end(), "Couldn't get him.");
       *      }
       *  }
       *  SNAX_DISPATCH( addressbook, (myaction) )
       *  @endcode
       ...
       *        snax::check(itr != addresses.end(), "Address for account not found");
       *        addresses.erase( itr );
       *        snax::check(itr != addresses.end(), "Everting lock arf, Address not erased properly");
       *      }
       *  }
       *  SNAX_ABI( addressbook, (myaction) )
       *  @endcode
       */
      const_iterator erase( const_iterator itr ) {
         snax::check( itr != end(), "cannot pass end iterator to erase" );
       ...
       *      void myaction() {
       *        auto itr = addresses.find("dan"_n);
       *        snax::check(itr != addresses.end(), "Record is not found");
       *        addresses.erase(*itr);
       *        itr = addresses.find("dan"_n);
       *        snax::check(itr == addresses.end(), "Record is not deleted");
       *      }
       *  }
       *  SNAX_DISPATCH( addressbook, (myaction) )
       *  @endcode
      ...
      void erase( const T& obj ) {
         ...
         const auto& objitem = static_cast<const item&>(obj);
         snax::check( objitem.__idx == this, "object passed to erase is not in multi_index" );
         snax::check( _code == current_receiver(), "cannot erase objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
         ...
         snax::check( itr2 != _items_vector.rend(), "attempt to remove object that was not in multi_index" );
