...
#warning "<snaxlib/binary_extension.hpp> is deprecated use <snax/binary_extension.hpp>"
 ...
 namespace snax {
   ...
   class binary_extension {
      public:
         ...
         constexpr T& value()& {
            if (!_has_value) {
               snax_assert(false, "cannot get value of empty binary_extension");
            }
            return _get();
         }
          /** get the contained value */
         ...
         **/
         ...
         friend inline DataStream& operator<<(DataStream& ds, const snax::binary_extension<T>& be) {
            ds << be.value_or();
            return ds;
         }
         ...
         /**
           ...
           */
         template<typename DataStream>
         friend inline DataStream& operator>>(DataStream& ds, snax::binary_extension<T>& be) {
            if( ds.remaining() ) {
               T val;
               ds >> val;
               be.emplace(val);
