...
#warning "<snaxlib/asset.hpp> is deprecated use <snax/asset.hpp>"
...
namespace snax {
   ...
   */
   ...
   struct asset {
      ...
      :amount(a),symbol{s}
      {
         snax::check( is_amount_within_range(), "magnitude of asset amount must be less than 2^62" );
         snax::check( symbol.is_valid(),        "invalid symbol name" );
      }
      ...
      asset& operator-=( const asset& a ) {
         snax::check( a.symbol == symbol, "attempt to subtract asset with different symbol" );
         amount -= a.amount;
         snax::check( -max_amount <= amount, "subtraction underflow" );
         snax::check( amount <= max_amount,  "subtraction overflow" );
         return *this;
      }
       ...
       */
      asset& operator+=( const asset& a ) {
         snax::check( a.symbol == symbol, "attempt to add asset with different symbol" );
         amount += a.amount;
         snax::check( -max_amount <= amount, "addition underflow" );
         snax::check( amount <= max_amount,  "addition overflow" );
         return *this;
      }
      ...
      asset& operator*=( int64_t a ) {
         ...
         snax::check( tmp <= max_amount, "multiplication overflow" );
         snax::check( tmp >= -max_amount, "multiplication underflow" );
         amount = (int64_t)tmp;
         return *this;
      }
       ...
       */
      asset& operator/=( int64_t a ) {
         snax::check( a != 0, "divide by zero" );
         snax::check( !(amount == std::numeric_limits<int64_t>::min() && a == -1), "signed division overflow" );
         amount /= a;
         return *this;
      }
      ...
      friend int64_t operator/( const asset& a, const asset& b ) {
         snax::check( b.amount != 0, "divide by zero" );
         snax::check( a.symbol == b.symbol, "comparison of assets with different symbols is not allowed" );
   ...
   */
   struct extended_asset {
      ...
      friend extended_asset operator - ( const extended_asset& a, const extended_asset& b ) {
         snax::check( a.contract == b.contract, "type mismatch" );
      ...
      SNAXLIB_SERIALIZE( extended_asset, (quantity)(contract) )
   };
