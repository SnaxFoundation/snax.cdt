#include <snax/action.h>
#include <snax/chain.h>
#include <snax/crypto.h>
#include <snax/db.h>
#include <snax/permission.h>
#include <snax/print.h>
#include <snax/privileged.h>
#include <snax/system.h>
#include <snax/transaction.h>
#include <snax/types.h>
#include "native/snax/intrinsics.hpp"
#include "native/snax/crt.hpp"
...
extern "C" {
   ...
   int32_t _snax_f32_trunc_i32s( float af ) {
      ...
      if (_snax_f32_ge(af, 2147483648.0f) || _snax_f32_lt(af, -2147483648.0f))
         snax_assert(false,  "Error, f32.convert_s/i32 overflow" );

      if (f32_is_nan(a))
         snax_assert(false,  "Error, f32.convert_s/i32 unrepresentable");
   ...
   int32_t _snax_f64_trunc_i32s( double af ) {
      ...
      if (_snax_f64_ge(af, 2147483648.0) || _snax_f64_lt(af, -2147483648.0))
         snax_assert(false,  "Error, f64.convert_s/i32 overflow");
      if (f64_is_nan(a))
         snax_assert(false,  "Error, f64.convert_s/i32 unrepresentable");
   ...
   uint32_t _snax_f32_trunc_i32u( float af ) {
      ...
      if (_snax_f32_ge(af, 4294967296.0f) || _snax_f32_le(af, -1.0f))
         snax_assert(false,  "Error, f32.convert_u/i32 overflow");
      if (f32_is_nan(a))
         snax_assert(false,  "Error, f32.convert_u/i32 unrepresentable");
   ...
   uint32_t _snax_f64_trunc_i32u( double af ) {
      ...
      if (_snax_f64_ge(af, 4294967296.0) || _snax_f64_le(af, -1.0))
         snax_assert(false,  "Error, f64.convert_u/i32 overflow");
      if (f64_is_nan(a))
         snax_assert(false,  "Error, f64.convert_u/i32 unrepresentable");
   ...
   int64_t _snax_f32_trunc_i64s( float af ) {
      ...
      if (_snax_f32_ge(af, 9223372036854775808.0f) || _snax_f32_lt(af, -9223372036854775808.0f))
         snax_assert(false,  "Error, f32.convert_s/i64 overflow");
      if (f32_is_nan(a))
         snax_assert(false,  "Error, f32.convert_s/i64 unrepresentable");
   ...
   int64_t _snax_f64_trunc_i64s( double af ) {
      ...
      if (_snax_f64_ge(af, 9223372036854775808.0) || _snax_f64_lt(af, -9223372036854775808.0))
         snax_assert(false,  "Error, f64.convert_s/i64 overflow");
      if (f64_is_nan(a))
         snax_assert(false,  "Error, f64.convert_s/i64 unrepresentable");
   ...
   uint64_t _snax_f32_trunc_i64u( float af ) {
      ...
      if (_snax_f32_ge(af, 18446744073709551616.0f) || _snax_f32_le(af, -1.0f))
         snax_assert(false,  "Error, f32.convert_u/i64 overflow");
      if (f32_is_nan(a))
         snax_assert(false,  "Error, f32.convert_u/i64 unrepresentable");
   ...
   uint64_t _snax_f64_trunc_i64u( double af ) {
      ...
      if (_snax_f64_ge(af, 18446744073709551616.0) || _snax_f64_le(af, -1.0))
         snax_assert(false,  "Error, f64.convert_u/i64 overflow");
      if (f64_is_nan(a))
         snax_assert(false,  "Error, f64.convert_u/i64 unrepresentable");
   ...
   void snax_assert(uint32_t test, const char* msg) {
      if (test == 0) {
         _prints(msg, snax::cdt::output_stream_kind::std_err);
         _prints_l("\n", 1, snax::cdt::output_stream_kind::none);
         longjmp(*___env_ptr, 1);
      }
   }

   void snax_assert_message(uint32_t test, const char* msg, uint32_t len) {
      if (test == 0) {
         _prints_l(msg, len, snax::cdt::output_stream_kind::std_err);
         _prints_l("\n", 1, snax::cdt::output_stream_kind::none);
         longjmp(*___env_ptr, 1);
      }
   }
   ...
   void snax_assert_code(uint32_t test, uint64_t code) {
      if (test == 0) {
         ...
         _prints(buff, snax::cdt::output_stream_kind::std_err);
         _prints_l("\n", 1, snax::cdt::output_stream_kind::none);
         longjmp(*___env_ptr, 1);
      }
   }
