...
### snaxlib
#### Partitioning
In `snax.cdt` v1.6.0, `snaxlib` will now be partitioned into 3 groups. These allow for finer grained allowance for particular modes of compilation.
  - CAPI
  - Contracts
  - Core
  - The `CAPI` will only be available to contract writers using `snax-cc` and purely for `C` smart contracts.
  - The `Contracts` will be available to `snax-cpp` for smart contract writing. 
  - `Core` will be available to `snax-cpp` for any of the modes (present and future).  
  - `snax-cpp` in `fnative` mode will have access to all of these groups.

To access these new partitioned header files, use `<snax/header_file.hpp>` instead of `<snaxlib/header_file.hpp>`.  Please note that all the old header files are still available at the old `snaxlib` directory, but these are deprecated and will be removed in v1.7.0.  Also, once you change one header file from `snaxlib` to `snax` you will need to do so for all other occurrences at that point, because of some conflicts with the auto generated dispatcher.
...
This entire API is now only available to C developers (i.e. using `snax-cc`), and all internal uses in `Core` and `Contracts` have been guarded behind the namespace `snax::internal_use_do_not_use`.  

### snaxlib Contracts API
- `action.hpp`
...
- `print.hpp`
   ...
   - removed explicit `print` overloads for `snax::symbol_code`, `snax::fixed_bytes<T>` from here.
- `rope.hpp`
   - new file
...
There is no more need to add the `SNAX_DISPATCH` macro to your smart contracts.  The compiler/linker will now automatically generate a dispatcher for you the proper `snax::contract`, `snax::action` and `snax::on_notify` attributes are used.  Of course, if you don't have these attributes then you will still need to either use the old macro or hand write the `apply` function yourself.

#### How the auto dispatcher will work
Given that you have marked your classes with the `snax::contract` macro and any sub-contracts with the macro with the same given name (i.e. `snax::contract("<contract name>")`) then any actions and notify handlers that are contained within these will be dispatchable by your smart contract.  This will allow for aggregate patterns for smart contract development and better separation of concerns. 
...
If the dispatcher is in notification handling mode and if your contract receives an `snax::onerror` notification, then the contract will assert with an error code. You can circumvent this check if you explicitly supply an error handler for it ([[snax::on_notify("snax::onerror")]]).
