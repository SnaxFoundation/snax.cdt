...
#include <snax/gen.hpp>

#include <snax/utils.hpp>
#include <snax/whereami/whereami.hpp>
#include <snax/abi.hpp>
...
using namespace snax::cdt;

namespace snax { namespace cdt {
   ...
   struct codegen_exception : public std::exception {
      virtual const char* what() const throw() {
         return "snax.codegen fatal error";
      }
   };
   ...
   // remove after v1.7.0
   bool has_snaxlib = false;

   class snax_ppcallbacks : public PPCallbacks {
      public:
         snax_ppcallbacks(SourceManager& sm, std::string file) : sources(sm), fn(file) {}
      protected:
         virtual void InclusionDirective(
            ...
            if ( file_name.find("snaxlib") != StringRef::npos )
               has_snaxlib = true;
         }
   ...
   class snax_codegen_visitor : public RecursiveASTVisitor<snax_codegen_visitor>, public generation_utils {
      private:
      ...
      public:
         ...
         explicit snax_codegen_visitor(CompilerInstance *CI)
         ...
         bool is_datastream(const QualType& qt) {
            ...
            auto ds_re    = std::regex("(((class snax::)?datastream<[a-zA-Z]+[a-zA-Z0-9]*.*>)|(DataStream)) &");
         ...
         */
         ...
         void create_dispatch(const std::string& attr, const std::string& func_name, F&& get_str, CXXMethodDecl* decl) {
            ...
            if (cg.is_snax_contract(decl, cg.contract_name)) {
               if (has_snaxlib) {
                  ss << "\n\n#include <snaxlib/datastream.hpp>\n";
                  ss << "#include <snaxlib/name.hpp>\n";
               } else {
                  ss << "\n\n#include <snax/datastream.hpp>\n";
                  ss << "#include <snax/name.hpp>\n";
               }
               ss << "extern \"C\" {\n";
               ...
               ss << "}\n";
               ss << "snax::datastream<const char*> ds{(char*)buff, as};\n";
               int i=0;
               ...
               ss << decl->getParent()->getQualifiedNameAsString() << "{snax::name{r},snax::name{c},ds}." << decl->getNameAsString() << "(";
         ...
         void create_action_dispatch(CXXMethodDecl* decl) {
            ...
            create_dispatch("snax_wasm_action", "__snax_action_", func, decl);
         }

         void create_notify_dispatch(CXXMethodDecl* decl) {
            auto func = [](CXXMethodDecl* d) { return generation_utils::get_notify_pair(d); };
            create_dispatch("snax_wasm_notify", "__snax_notify_", func, decl);
         }
         ...
         virtual bool VisitCXXMethodDecl(CXXMethodDecl* decl) {
            ...
            if (decl->isSnaxAction()) {
               name = generation_utils::get_action_name(decl);
               validate_name(name, [&]() {emitError(*ci, decl->getLocation(), "action not a valid snax name");});
      ...
      class snax_codegen_consumer : public ASTConsumer {
      private:
         snax_codegen_visitor *visitor;
         std::string main_file;
         CompilerInstance* ci;

      public:
         explicit snax_codegen_consumer(CompilerInstance *CI, std::string file)
            : visitor(new snax_codegen_visitor(CI)), main_file(file), ci(CI) { }
         ...
         virtual void HandleTranslationUnit(ASTContext &Context) {
            ...
            if (main_fe) {
               ...
               try {
                  ...
                  std::stringstream ss;
                  ss << "extern \"C\" {\n";
                  ss << "void snax_assert_code(uint32_t, uint64_t);";
                  ss << "\t__attribute__((weak, snax_wasm_entry, snax_wasm_abi(";
                  std::string abi = cg.abi;
                  ss << "\"" << _quoted(abi) << "\"";
                  ss << ")))\n";
                  ss << "\tvoid __insert_snax_abi(unsigned long long r, unsigned long long c, unsigned long long a){";
                  ss << "snax_assert_code(false, 1);";
                  ss << "}\n";
                  ss << "}";
      ...
      class snax_codegen_frontend_action : public ASTFrontendAction {
      public:
         virtual std::unique_ptr<ASTConsumer> CreateASTConsumer(CompilerInstance &CI, StringRef file) {
            CI.getPreprocessor().addPPCallbacks(_make_unique<snax_ppcallbacks>(CI.getSourceManager(), file.str()));
            return _make_unique<snax_codegen_consumer>(&CI, file);
         }
   };
