...
struct generation_utils {
      ...
      auto get = [&](const clang::Type* pt) {
         if (auto tst = llvm::dyn_cast<clang::TemplateSpecializationType>(pt))
            if (auto decl = llvm::dyn_cast<clang::RecordType>(tst->desugar()))
               return decl->getDecl()->isSnaxIgnore() ? tst->getArg(0).getAsType() : type;
         return type;
      };
   ...
   static inline bool has_snax_ricardian( const clang::CXXMethodDecl* decl ) {
      return decl->hasSnaxRicardian();
   }
   static inline bool has_snax_ricardian( const clang::CXXRecordDecl* decl ) {
      return decl->hasSnaxRicardian();
   }

   static inline std::string get_snax_ricardian( const clang::CXXMethodDecl* decl ) {
      return decl->getSnaxRicardianAttr()->getName();
   }
   static inline std::string get_snax_ricardian( const clang::CXXRecordDecl* decl ) {
      return decl->getSnaxRicardianAttr()->getName();
   }

   static inline std::string get_action_name( const clang::CXXMethodDecl* decl ) {
      std::string action_name = "";
      auto tmp = decl->getSnaxActionAttr()->getName();
      if (!tmp.empty())
         return tmp;
   ...
   static inline std::string get_notify_pair( const clang::CXXMethodDecl* decl ) {
      ...
      auto tmp = decl->getSnaxNotifyAttr()->getName();
      return tmp;
   }
   ...
   static inline bool is_snax_contract( const clang::CXXMethodDecl* decl, const std::string& cn ) {
      std::string name = "";
      if (decl->isSnaxContract())
         name = decl->getSnaxContractAttr()->getName();
      else if (decl->getParent()->isSnaxContract())
         name = decl->getParent()->getSnaxContractAttr()->getName();
      if (name.empty()) {
   ...
   static inline bool is_snax_contract( const clang::CXXRecordDecl* decl, const std::string& cn ) {
      std::string name = "";
      auto pd = llvm::dyn_cast<clang::CXXRecordDecl>(decl->getParent());
      if (decl->isSnaxContract()) {
         auto nm = decl->getSnaxContractAttr()->getName().str();
         name = nm.empty() ? decl->getName().str() : nm;
      }
      else if (pd && pd->isSnaxContract()) {
         auto nm = pd->getSnaxContractAttr()->getName().str();
